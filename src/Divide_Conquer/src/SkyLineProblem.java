...
public class SkyLineProblem
{
	static class BuildingPoint implements Comparable<BuildingPoint>
	{
		...
		{
			//whoever is horizontally first, should come first
			if(this.x<o.x)
				return -1;
			else if(this.x>o.x)
				return 1;

			//below is when both point x position both points are start
			...
			//below is when both point x position both points are ends
			if(!this.isStart&&!o.isStart)
			{
				if(this.height>o.height)
					return -1;
			...
			// When the same point is start for one and end for another
			if(this.isStart)
				return 1;
			return -1;
		}
