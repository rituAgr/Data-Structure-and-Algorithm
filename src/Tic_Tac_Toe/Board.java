...
//Setting start board
    ...
    void MakeMove(int choice) throws IOException
    {
        //1 -> human ; 2 -> Computer
        if(choice==1)
        {   if(!pMoves.make_move(gameBoard,playerMoves))
            return;
            gameOver=true;
            intelligence.UpdateFailedMoves(computerMoves);// Updating computer moves if it loses
        }
        else
        {
            if(!cMoves.make_move(gameBoard,computerMoves,playerMoves ))
                return;
            gameOver=true;
        }
    }
