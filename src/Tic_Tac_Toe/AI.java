...
public class AI
{
  ...
  //This function with help of other helper func relative position of given position
    ...
    void UpdateFailedMoves(ArrayList<position> computerMoves)throws IOException
  {
...
//Writing the failed move into file
  void UpdateOnto_File(position[] pos)throws IOException
  {
      ...
      str.append("*");
      try {
          FileWriter writer = new FileWriter("Loss Moves", true);
    ...
    HashSet<position> failingMoves(ArrayList<position> moves)throws IOException
    {
        HashSet<position> res=new HashSet<>();
        if(moves.size()==0)
            return res;
        try(BufferedReader br = new BufferedReader(new FileReader("Loss Moves")))
        {
            String line;
